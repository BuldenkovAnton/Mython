...
namespace parse {
    ...
    private:
        ...
        size_t token_current_index_ = 0;


        const std::unordered_map<std::string_view, Token> keywords_ = {
        ...
        bool IsId(char token) {
            if (std::isalpha(token) || std::isdigit(token) || token == '_') return true;
            return false;
        }
